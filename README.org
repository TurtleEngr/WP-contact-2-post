...
- Post author is 0 (no user). Content is also filtered by core kses,
  because the submitter does not have unfiltered_html.
- If there is no publish-date, the post date "floats": it is set when
